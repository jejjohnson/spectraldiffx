...
class FourierGrid1D(eqx.Module):
    ...
    L: float
    dx: float
    dealias: Optional[Literal["2/3", "padding", None]] = "2/3"
    ...
    @classmethod
    def from_N_L(cls, N: int, L: float, dealias: Optional[Literal["2/3", "padding", None]] = "2/3") -> "FourierGrid":
        """
    ...
    @classmethod
    def from_N_dx(cls, N: int, dx: float, dealias: Optional[Literal["2/3", "padding", None]] = "2/3") -> "FourierGrid":
        """
    ...
    @classmethod
    def from_L_dx(cls, L: float, dx: float, dealias: Optional[Literal["2/3", "padding", None]] = "2/3") -> "FourierGrid":
        """
        ...
        if not jnp.isclose(N_float % 1, 0) and not jnp.isclose(N_float % 1, 1):
             raise ValueError(f"L={L} is not divisible by dx={dx}. Resulting N={N_float} is not an integer.")
...
class FourierGrid2D(eqx.Module):
    ...
    def check_consistency(self, rtol: float = 1e-5) -> bool:
        ...
        if not jnp.isclose(self.Lx, expected_Lx, rtol=rtol):
            errors.append(f"X-dimension inconsistency: Lx={self.Lx} vs Nx*dx={expected_Lx}")
        if not jnp.isclose(self.Ly, expected_Ly, rtol=rtol):
            errors.append(f"Y-dimension inconsistency: Ly={self.Ly} vs Ny*dy={expected_Ly}")
    ...
    def X(self) -> Tuple[Float[Array, "Ny Nx"], Float[Array, "Ny Nx"]]:
        """2D meshgrid: X, Y"""
        return jnp.meshgrid(self.x, self.y, indexing='xy')
    ...
    def KX(self) -> Tuple[Float[Array, "Ny Nx"], Float[Array, "Ny Nx"]]:
        ...
        return jnp.meshgrid(self.kx, self.ky, indexing='xy')
...
class FourierGrid3D(eqx.Module):
    ...
    def check_consistency(self, rtol: float = 1e-5) -> bool:
        ...
        if not jnp.isclose(self.Lz, expected_Lz, rtol=rtol):
            errors.append(f"Z-dimension inconsistency: Lz={self.Lz} vs Nz*dz={expected_Lz}")
        if not jnp.isclose(self.Ly, expected_Ly, rtol=rtol):
            errors.append(f"Y-dimension inconsistency: Ly={self.Ly} vs Ny*dy={expected_Ly}")
        if not jnp.isclose(self.Lx, expected_Lx, rtol=rtol):
            errors.append(f"X-dimension inconsistency: Lx={self.Lx} vs Nx*dx={expected_Lx}")
    ...
    @property
    def X(self) -> Tuple[Float[Array, "Nz Ny Nx"], Float[Array, "Nz Ny Nx"], Float[Array, "Nz Ny Nx"]]:
        """
        3D meshgrid: Z, Y, X
        ...
        """
        return jnp.meshgrid(self.z, self.y, self.x, indexing='ij')
    ...
    @property
    def KX(self) -> Tuple[Float[Array, "Nz Ny Nx"], Float[Array, "Nz Ny Nx"], Float[Array, "Nz Ny Nx"]]:
        """
        ...
        Resulting shape: (Nz, Ny, Nx).
        """
        return jnp.meshgrid(self.kz, self.ky, self.kx, indexing='ij')
